...
async function isAdmin(chatId, username) {
    try {
        ...
        const response = await axios.get('https://caliweed.onrender.com/admins');
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: 'SHOP',
                            web_app: { url: `https://caliweed.onrender.com/admin/admin.html?userId=${chatId}` }
                        }
                    ]
                ]
            }
...
bot.onText(/\/addpromocode (\w+) (\d+)/, async (msg, match) => {
    await checkAdminAndExecute(msg, async (ctx) => {
        ...
        try {
            const response = await axios.post('https://caliweed.onrender.com/promocodes', { code, amount });
...
bot.onText(/\/deletepromocode (\w+)/, async (msg, match) => {
    await checkAdminAndExecute(msg, async (ctx) => {
        ...
        try {
            const response = await axios.delete('https://caliweed.onrender.com/promocodes', { data: { code } });
            if (response.data.success) {
...
bot.onText(/\/promocodes/, async (msg) => {
    await checkAdminAndExecute(msg, async (ctx) => {
        ...
        try {
            const response = await axios.get('https://caliweed.onrender.com/promocodes');
            if (response.data.success) {
...
bot.onText(/\/addadmin (\w+)/, async (msg, match) => {
    await checkAdminAndExecute(msg, async (ctx) => {
        ...
        try {
            ...
            const response = await axios.post('https://caliweed.onrender.com/admins', {
                action: 'add',
                user_id: userId
            });
